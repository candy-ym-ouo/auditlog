...
func (x *API) context(w http.ResponseWriter, r *http.Request) {
	...
	if requestID := r.Header.Get("X-Request-ID"); requestID != "" {
		v.ChainPosition["request_id"] = requestID
	}
	writeJSON(w, 200, v)
}
...
func (x *API) archive(w http.ResponseWriter, r *http.Request) {
	...
	// The HTTP request context is canceled as soon as this response is sent.
	// Archive runs asynchronously, so it needs an independent lifecycle.
	go x.Archive.Trigger(context.Background())
